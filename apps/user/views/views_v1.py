...
from apps.user.serializers.serializers_v1 import UserCreateSerializer, UserUpdateSerializer, UserListSerializer, \
...
class UserViewSet(mixins.CreateModelMixin,
    ...
    def get_queryset(self):
        return self.model_class.objects.filter(is_active=True).exclude(role='super_user')
    ...
    def owner_signup(self, request, *args, **kwargs):
        if self.model_class.objects.filter(email=request.data['email']).first():
            return Response({'detail': 'This email is taken by another user.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if 'password' in request.data.keys():
            try:
                ...
                return Response({'detail': 'Given password is too weak.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'detail': 'Registration success.'}, status=status.HTTP_201_CREATED)
        else:
    ...
    def put(self, request, *args, **kwargs):
        ...
        if self.model_class.objects.filter(~Q(id=instance.id), email=request.data['email']).first():
            return Response({'detail': 'This email is taken by another user.'}, status=status.HTTP_400_BAD_REQUEST)
        if self.model_class.objects.filter(~Q(id=instance.id), username=request.data['username']).first():
            return Response({'detail': 'Username is already in use.'}, status=status.HTTP_400_BAD_REQUEST)

        if 'password' in request.data.keys():
            return Response({'detail': 'Invalid request. Password change restricted.'},
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'detail': 'Information updated.'}, status=status.HTTP_201_CREATED)
        else:
            return Response({'detail': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
